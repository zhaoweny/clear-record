...
background service behind a menu-bar icon. Options are the `tray` subcommand's,
so ``clear-record-tray --port 9000 --no-browser`` works; the separate
`clear-record-web` binary starts the console without a tray, and `clear-record`
is the full CLI.
"""
