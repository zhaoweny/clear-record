...
from cr_providers import get_backend, probe_ggml_plugin_load

from cr_cli import eval as _eval
from cr_cli import transcription
from cr_cli.workspace import Workspace, discover_audio
...
def transcribe(
    ...
    :mod:`cr_cli.transcription`. Here we load the manifest, gate on the backend
    and the opt-in plugin probe, resolve the model once (single-threaded, before
    the pool), then persist and print the result.
    """
    w = Workspace.at(directory)
    sources, _ = w.load_manifest()
