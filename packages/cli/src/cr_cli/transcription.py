...
``cr_cli.stages.transcribe`` is only wiring: it loads the manifest, resolves the
backend and model, then calls :func:`transcribe` here. Everything the tests need
...
from cr_providers import CancellableProcessRunner

from cr_cli.workspace import ChunkCache, Workspace, chunk_cache_key
