...
// The signed-in session `seed.py` mints, handed to every spec as the
// `storageState` in playwright.config.ts: the console answers pages and API
// alike only to a live session (ADR-0033). Written beside the data directory,
// like the run owner's pid file, and regenerated by every run.
export const sessionStatePath = resolve(dirname(dataDir), "console-session.json");
